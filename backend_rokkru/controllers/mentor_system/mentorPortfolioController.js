...
import { portfolioUploadDir } from '../../middleware/mentor_system/portfolioUpload.js';
...
import { PORTFOLIO_MAX_FILES_PER_ITEM } from '../../middleware/mentor_system/portfolioUpload.js';
import {
...
} from '../../utils/mentor_system/portfolioHelpers.js';
...
async function findPortfolioItem(mentorId, link) {
  return MentorPortfolio.findOne({ where: { mentor_id: mentorId, link } });
}
...
const creatPortfolio = async (req, res) => {
  try {
    ...
    const existing = await MentorPortfolio.findOne({
      where: { mentor_id: mentorId, link: built.value.link },
    });
    if (existing) {
